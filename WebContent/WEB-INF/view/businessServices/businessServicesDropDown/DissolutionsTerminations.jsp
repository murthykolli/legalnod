<%--
    Document   : DissolutionsTerminations
...
<html>    
    ...
    newAjaxCallForCategory();
    } });

    function validation(){
    var stateName = document.busStateFormsName.stateName.value;
    var serviceByDocCategory = document.busStateFormsName.categoryName.value;
    ...
    if(stateName !== ""){
    document.getElementById("statelabel").removeAttribute("class");
    var categoryName = "Dissolutions Terminations";
    ...
    function selectDocCateAndFormBeforeState(){
    if(document.busStateFormsName.stateName.value === ""){
    alert("Please select a state first");
    } else if(document.busStateFormsName.categoryName.value === ""){
    ...
    function newAjaxCallForCategorySelection(selectedstate){   
    	$.ajax({
			type : 'POST',
			url : "jSonCatSelectionWithState?stateName="+selectedstate,
			dataType : 'json',
			async : true,
			cache: false,
    ...
    function newAjaxCallForFormSelection(selectedstate){   
    	$.ajax({
			type : 'POST',
			url : "jSonFormSelectionWithCatAndState?stateName="+selectedstate,
			dataType : 'json',
			async : true,
			cache: false,
			success : function(result) { 
    ...
    function newAjaxPriceCall(selectedData){     	
		...
		<div class="col-md-12">
		<div class="white-background" style="min-height: 620px;height: auto;">
		<div class="servicesbystate">
		<div>
		<div class="services-left">
		<h2 class="busServHeading"> Dissolutions/Terminations</h2>
		<p class="bs-dropdown-LMText"> Learn more about Dissolutions/Terminations at the  &nbsp;<a href="DissolutionsTerminationsDoc" onMouseOver="this.style.color='#3c3c3c';" onMouseOut="this.style.color='#00a9f1';">DOC CENTER</a></p>
		...
		<c:forEach var="statesList" items="${stateList}"> <option value="${statesList.stateName}">${statesList.stateName}</option>
		</c:forEach>	        
		...
		<tr><td><select id="categoryNameId" name="categoryName" onchange="llcCategoiesValidation();" onclick="selectDocCateBeforeForm();">
	    <option value="">--------Select Category Name--------</option>
		</select></td></tr> <tr style="height: 15px;"></tr>		
		
		<tr class="bigFormLabel"><td id="formlabel">I want to form a: </td></tr> <tr style="height: 5px;"></tr>
		<tr><td><select id="formNameId" name="formName" onchange="llcFormNameValidation();" onclick="selectDocCateAndFormBeforeState();">
