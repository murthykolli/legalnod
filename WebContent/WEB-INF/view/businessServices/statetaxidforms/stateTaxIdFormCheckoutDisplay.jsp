<%--
    Document   : stateTaxIdFormCheckoutDisplay
...
<html>    
    ...
    function newAjaxCall(selectedData){    	
    	$.ajax({
			type : 'POST',
			url : "jSonSTIModifiedValuesUpdate?userChoice="+selectedData,
			dataType : 'json',
			async : true,
			cache: false,
        ...
        if(null != session.getAttribute("Attr_IDs_STI_CheckOutList")) {
        ArrayList attIdsList = (ArrayList) session.getAttribute("Attr_IDs_STI_CheckOutList");
        if (null != attIdsList && attIdsList.size() > 0) {

        if(null != session.getAttribute("Attr_STI_Type_CheckOutList")) {
        ArrayList attTypesList = (ArrayList) session.getAttribute("Attr_STI_Type_CheckOutList");
		...
		<form action="stateTaxIdCheckoutDataModification" name="psDocmentName" id="pro-form">            
		...
		<input type="hidden" name="stateTaxIdFormsCheckOutRef"/>
        </form> 
        <div class="clearfix"></div>
		</div> </div>
        ...
        document.psDocmentName.stateTaxIdFormsCheckOutRef.value = "STICheckoutModification";
        }
        	
        function goToCheckout(){
        document.psDocmentName.stateTaxIdFormsCheckOutRef.value = "STICheckoutPayment";
        }
        </script>
