<%--
    Document   : Free_Federal_With_StateForms_Checkout_Display
...
<html>    
    ...
    $("#"+sbHide).hide();
    $("#"+sbStateID).show();
    if(sbTypeId === "SB49" || sbTypeId === "SB50"){
    var rbHide1 = "RbHideID"+i;
    $("#"+rbHide1).hide();
    }
    break;
    ...
    function newAjaxCall(selectedData){    	
    	$.ajax({
			type : 'POST',
			url : "jSonModifiedValuesUpdate?userChoice="+selectedData,
			dataType : 'json',
			async : true,
			cache: false,
        ...
        if(null != session.getAttribute("Attr_IDs_SF_CheckOutList")) {
        ArrayList attIdsList = (ArrayList) session.getAttribute("Attr_IDs_SF_CheckOutList");
        if (null != attIdsList && attIdsList.size() > 0) {

        if(null != session.getAttribute("Attr_SF_Type_CheckOutList")) {
        ArrayList attTypesList = (ArrayList) session.getAttribute("Attr_SF_Type_CheckOutList");
		...
		<form action="freeFedWithStateFormscheckoutDataModification" name="checkoutDisplayForm" id="pro-form" method="get">            
		...
		<button class="ord-wt-bg-sub-but btn-height" type="submit" onclick="return goToFormFederalCheckoutOrder();" onMouseOver="document.getElementById('chek-sub-but').src='resources/images/SSImgs/RegPointerWhiteImg.png';" onMouseOut="document.getElementById('chek-sub-but').src='resources/images/SSImgs/RegPointerBlueImg.png';" ><img id="chek-sub-but" src="resources/images/SSImgs/RegPointerBlueImg.png" alt=""><span class="butt-va-mid">Finish</span></button>
		</h4>
		
		<input type="hidden" name="stateAndFreeFedCheckOutRef"/>
        </form> 
        <div class="clearfix"></div>
		</div> </div>
        ...
        document.checkoutDisplayForm.stateAndFreeFedCheckOutRef.value = "CheckoutModification";
        }
    	
    	function goToFormFederalCheckoutOrder(){
    	document.checkoutDisplayForm.stateAndFreeFedCheckOutRef.value = "FormFederalCheckout";
        }
        </script>
